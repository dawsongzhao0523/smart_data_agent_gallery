...
class ChatGPT_Handler: #designed for chatcompletion API
    def __init__(self, extract_patterns=None,client=None,gtp_model=None,max_response_tokens=None,token_limit=None,temperature=None ) -> None:
        self.client = client
        self.max_response_tokens = max_response_tokens
        self.token_limit= token_limit
        self.gtp_model=gtp_model
        self.temperature=temperature
        self.extract_patterns = extract_patterns
        # self.conversation_history = []
    def _call_llm(self,prompt, stop):
        response = self.client.chat.completions.create(
        model=self.gtp_model, 
        messages = prompt,
    ...
    #  从输入文本中提取代码和注释
    #  TODO gpt4o支持100%结构化输出了，未来这个函数可以重构
        ...
        try:
            ...
            while n<2:
                try:
                    ...
                    print(f"error calling open AI, I am retrying 2 attempts , attempt {n}")
                    time.sleep(8) #sleep for 8 seconds
                    print(str(e))
